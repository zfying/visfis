...
if __name__ == '__main__':
    ...
    train_answer_file = f'{data_dir}/questions/v2_mscoco_train2014_annotations.json'
    train_answers = json.load(open(train_answer_file))['annotations']

    val_answer_file = f'{data_dir}/questions/v2_mscoco_val2014_annotations.json'
    val_answers = json.load(open(val_answer_file))['annotations']

    train_question_file = f'{data_dir}/questions/v2_OpenEnded_mscoco_train2014_questions.json'
    train_questions = json.load(open(train_question_file))['questions']

    val_question_file = f'{data_dir}/questions/v2_OpenEnded_mscoco_val2014_questions.json'
    val_questions = json.load(open(val_question_file))['questions']

    answers = train_answers + val_answers
    occurence = filter_answers(answers, 9)
    processed_dir = os.path.join(data_dir, 'processed')
    ans2label = create_ans2label(occurence, 'trainval', cache_root=processed_dir)
    compute_target(train_answers, ans2label, 'train', cache_root=processed_dir)
    compute_target(val_answers, ans2label, 'val', cache_root=processed_dir)